...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $expences = Transaction::whereHas('financeCategory', fn ($query) => $query->where('type', 'expense'))
            ->where('cooperative_id', $defaultCooperativeId)
            ->avg('amount');
        $incomes = Transaction::whereHas('financeCategory', fn ($query) => $query->where('type', 'income'))
            ->where('cooperative_id', $defaultCooperativeId)
            ->avg('amount');
